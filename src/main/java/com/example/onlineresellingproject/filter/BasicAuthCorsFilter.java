package com.example.onlineresellingproject.filter;
 ...
 *
 * <p>Этот фильтр позволяет браузерам выполнять запросы с учетными данными (например, куки или HTTP-авторизацией) на
 * сервер, даже если запрос отправляется с другого домена (origin). Он гарантирует, что сервер разрешает обмен данными
